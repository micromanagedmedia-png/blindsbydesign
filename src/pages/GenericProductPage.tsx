...
export default function GenericProductPage({ title, description, categories }: { title: string, description: string, categories: { name: string, desc: string }[] }) {
  return (
    <>
      ...
      {/* Hero */}
      <section className="relative py-24 bg-brand-teal text-white overflow-hidden border-b border-brand-teal-dark">
        <div className="absolute inset-0 bg-black/10 z-0"></div>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 flex flex-col items-center text-center">
          <div className="inline-flex items-center gap-2 mb-6">
            <span className="h-[1px] w-8 bg-teal-200"></span>
            <span className="text-xs font-bold tracking-[0.2em] uppercase text-teal-100">Premium Collection</span>
            <span className="h-[1px] w-8 bg-teal-200"></span>
          </div>
          <h1 className="text-5xl md:text-7xl font-extrabold mb-6 max-w-4xl tracking-tight uppercase">{title}</h1>
          <p className="text-lg md:text-xl font-medium text-teal-50 max-w-2xl leading-relaxed">
            {description}
          </p>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            {categories.map((cat, i) => (
              <div key={i} className="bg-white rounded-xl p-10 shadow-sm border border-slate-200 flex flex-col h-full hover:border-brand-teal transition-colors group">
                ...
                <Link to="/contact" className="inline-flex items-center space-x-2 text-brand-teal font-bold uppercase tracking-widest text-xs group-hover:text-brand-teal-dark transition-colors w-fit">
