    ...
    <>
      ...
      <section className="relative min-h-[85vh] bg-white border-b border-slate-100 flex items-center justify-center pt-24 pb-24 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 flex flex-col items-center text-center">
          <div className="inline-flex items-center gap-2 mb-6">
            ...
            <span className="text-xs font-bold tracking-[0.2em] uppercase text-brand-teal">Coastal Maine Professional</span>
            <span className="h-[1px] w-8 bg-brand-teal"></span>
          </div>
          ...
          <h1 className="text-5xl md:text-7xl lg:text-8xl font-extrabold text-slate-900 leading-[1.1] mb-6 max-w-5xl mx-auto tracking-tight">
            Design Your <span className="text-brand-teal uppercase">Perfect</span> Light.
          </h1>
          ...
          <div className="flex gap-4 mb-10 flex-wrap justify-center">
            <div className="flex items-center bg-stone-100 px-4 py-2 rounded-full border border-stone-200">
              <Star className="w-4 h-4 text-brand-yellow fill-current mr-2" />
              <span className="text-sm font-bold text-slate-700 uppercase tracking-wide">5.0 Google Rating</span>
            </div>
            <div className="flex items-center bg-teal-50 px-4 py-2 rounded-full border border-teal-100">
               <Shield className="w-4 h-4 text-brand-teal mr-2" />
               <span className="text-sm font-bold text-brand-teal uppercase tracking-wide">Child Safety Certified</span>
            </div>
          </div>

          <div className="flex gap-6 items-center flex-col sm:flex-row">
            <Link to="/contact" className="bg-brand-yellow text-slate-900 px-8 py-4 rounded text-sm font-bold shadow-sm hover:bg-yellow-400 transition-colors uppercase tracking-widest flex items-center justify-center space-x-2">
              <span>Free Consultation</span>
            </Link>
            <Link to="/shutters" className="text-sm font-bold text-brand-teal tracking-widest uppercase flex items-center gap-2 hover:opacity-80 transition-opacity">
               Explore Shutters ➔
            </Link>
          </div>
        </div>
      ...
      <section className="bg-white py-16 border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="bg-[#F7F9FB] rounded-xl border border-slate-200 p-8 flex flex-col items-center md:items-start text-center md:text-left hover:border-brand-teal transition-colors">
          ...
          <div className="bg-[#F7F9FB] rounded-xl border border-slate-200 p-8 flex flex-col items-center md:items-start text-center md:text-left hover:border-brand-teal transition-colors">
             <Shield className="w-8 h-8 text-brand-teal mb-4" />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              <Link key={service.path} to={service.path} className="bg-white rounded-xl shadow-sm border border-slate-200 p-8 flex flex-col hover:border-brand-teal transition-all group">
                <div className="w-12 h-12 bg-teal-50 rounded-lg flex items-center justify-center mb-6 group-hover:bg-brand-teal transition-colors">
                  <ArrowRight className="w-6 h-6 text-brand-teal group-hover:text-white group-hover:translate-x-1 transition-all" />
                </div>
                ...
                <span className="mt-auto text-xs font-bold text-brand-teal tracking-widest uppercase flex items-center gap-2">View ➔</span>
              </Link>
            ))}
          </div>
        </div>
